...
def generate_table(df, pdf_path):
    ...
    for i, row in enumerate(data):
        ...
        for j, cell in enumerate(row):
            # 针对title的
            if i < len_title and isinstance(cell, str) and len(cell) > nextline_num:
                data[i][j] = Paragraph(cell, table_title_style)
                
            if i >= len_title and isinstance(cell, str) and len(cell) > nextline_num:  # 将所有字符串都处理为Paragraph
                data[i][j] = Paragraph(cell, table_paragraph_style)  #如果cell里嵌套了表格则无法正确输出
    ...
    for row in data:
        for i, cell in enumerate(row):
            if isinstance(cell, Paragraph):
                # para_width, _ = cell.wrap(1, 1)
                para_width = np.random.randint(nextline_num * 10, (nextline_num+1)*10 )
                col_widths[i] = max(col_widths[i], para_width)
            else:
                col_widths[i] = max(col_widths[i], len(cell) * table_paragraph_style.fontSize)
                
                
    
    table = Table(data, colWidths=[i * max_width / sum(col_widths)  for i in col_widths])
...
def add_gaussian_blur(image):
    radius = np.random.randint(2, 10) * 0.1
...
def add_uniform_noise(image):
    ...
    noise = np.random.uniform(low=0, high=120, size=img_array.shape)
